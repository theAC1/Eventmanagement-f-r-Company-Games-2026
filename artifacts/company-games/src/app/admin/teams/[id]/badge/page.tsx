...
export default function BadgePage() {
  ...
  useEffect(() => {
    Promise.all([
      fetch(`/api/teams/${teamId}/badge`).then(r => r.json()),
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2 text-sm text-zinc-500">
          <Link to="/admin/teams" className="hover:text-white transition">Teams</Link>
          <span>/</span>
          <Link to={`/admin/teams/${team.id}`} className="hover:text-white transition">{team.name}</Link>
          <span>/</span>
  ...
  return (
    <div style={{ width: "380px", backgroundColor: "#0a0a0a", borderRadius: "16px", overflow: "hidden", pageBreakInside: "avoid" }}>
      ...
      <div style={{ padding: "24px 28px", display: "flex", flexDirection: "column", gap: "20px" }}>
        ...
        {/* Einziger QR-Code: Team-Portal (routet je nach Scanner) */}
        <div style={{ textAlign: "center" }}>
          <div style={{
