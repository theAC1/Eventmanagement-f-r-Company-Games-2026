...
export default function NewMaterialPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      navigate(`/admin/materials/${item.id}`);
    } catch (err) {
  ...
  return (
    <div className="max-w-2xl space-y-6">
      <div className="flex items-center gap-2 text-sm text-zinc-500">
        <Link to="/admin/materials" className="hover:text-white transition">Material</Link>
        <span>/</span>
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <div className="flex items-center gap-3">
          ...
          <Link to="/admin/materials" className="px-4 py-2 text-sm text-zinc-400 hover:text-white transition">
            Abbrechen
          </Link>
        </div>
      </form>
